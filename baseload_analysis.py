...
do_subset = 0
do_raw = False
do_baseload_lines = False
do_baseload_areas = False
do_hist = False
...
#    {'lbl': 'New Walk Museum', 'id': 75},                  #bad data
...
for dataset in datasets:
    ...
    dataset['weekly_percent'] = (dataset['weekly_min']['consumption (kWh)'] * 336) / dataset['weekly_sum']['consumption (kWh)']
...
if do_hist:
    ...
    for i, dataset in enumerate(datasets):    
        ...
        n, bins, patches = ax.hist(dataset['dataframe']['consumption (kWh)'], bins=nbins, label='total load', color='blue', alpha=0.5)
...
boxdata = []
datasets.sort(key=lambda ds: ds['weekly']['total']['median'])
for i, dataset in enumerate(datasets):
	boxdata.append(dataset['weekly_sum']['consumption (kWh)'])
...
ax.set_title ('weekly total consumption')
plt.tight_layout()
plt.savefig(os.path.join(output_folder, "Box Chart total.png"))
plt.close()	 

boxdata = []
datasets.sort(key=lambda ds: ds['weekly']['baseload']['median'])
for i, dataset in enumerate(datasets):
	boxdata.append(dataset['weekly_min']['consumption (kWh)'] * 336)
lbls = [''.join(c for c in d['lbl'] if is_number_or_cap(c)) for d in datasets]
...
plt.tight_layout()
plt.savefig(os.path.join(output_folder, "Box Chart baseload.png"))
plt.close()

boxdata = []
datasets.sort(key=lambda ds: ds['weekly']['baseload%'])
for i, dataset in enumerate(datasets):
	boxdata.append(dataset['weekly_percent'])
lbls = [''.join(c for c in d['lbl'] if is_number_or_cap(c)) for d in datasets]
...
# ax.xaxis.set_major_locator(loc)
# ax.xaxis.set_major_formatter(fmt)
ax.set_ylabel('weekly baseload / usage (%)')
ax.set_title ('weekly baseload percent')
plt.tight_layout()
plt.savefig(os.path.join(output_folder, "Box Chart percent.png"))
plt.close()
